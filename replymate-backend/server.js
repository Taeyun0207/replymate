...
// Translate text (requires auth). Free: 10/day. Pro: 30/day. Pro+: unlimited.
...
- ${toneInstructions}
- End with an appropriate closing. ${userName ? `Sign off with the name: "${userName}". Use this name exactly as written, regardless of the reply language.` : "The user's saved name is not set—do not infer or guess their name from the thread (ignore how they are addressed in incoming mail). Choose by context: (1) If a signature line fits (formal or typical business reply), end with a closing phrase (Best regards, / Thanks, / Sincerely, etc.) followed on the next line by the literal placeholder [Your Name] exactly, for the user to replace. (2) If the reply is very short or informal, end with only the closing/thanks and no name line. Never invent a real name."}
